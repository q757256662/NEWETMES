...
import { page, pageTable } from "api/admin/chart/CurrentYearStatistics";
export default {
  props: {
    className: {
      type: String,
  ...
  data() {
    return {
      ...
      chart: null,
      currenWeek: null,
      legend: ["样板数", "生产单数", "件数", "床数", "铺布总长度", "特体Pod数"],
      year1: "2018",
      xAxis: [
        "第2周",
        "第4周",
      ...
      if (response.status === 200) {
        if (response.data.type == "sql server") {
          ...
          for (let index = 0; index < array.length; index++) {
            ...
            var arr = [];
            for (
              let i = array[index].Data.length - this.currenWeek;
              i < array[index].Data.length;
              i++
            ) {
      ...
      this.chart.setOption({
        ...
        optionToContent: function(opt) {
          ...
          for (let i = 0, l = axisData.length; i < l; i++) {
            ...
            table += `<tr><td style="padding: 0 10px">${
              axisData[i]
            }</td>${tdBodys}</tr>`;
            tdBodys = "";
          }
